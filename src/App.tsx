...
import { 
  ...
  Users, MailCheck, ServerCrash, Map, Rocket, Clock, Share2, FileImage,
  Store, Activity, Wrench, Shirt, HelpCircle, ArrowRight, ChevronDown
} from 'lucide-react';

function App() {
  // Estado que controla qué pestaña se está mostrando actualmente
  ...
  return (
    <div className="min-h-screen bg-[#0c0a09] text-stone-300 font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      ...
      {/* 📱 BARRA DE NAVEGACIÓN FIJA (ESCRITORIO) 📱 */}
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#0c0a09]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-bold">
            <button onClick={() => setActiveTab('inicio')} className={`transition-colors ${activeTab === 'inicio' ? 'text-white border-b-2 border-indigo-500 pb-1' : 'hover:text-indigo-400'}`}>Inicio</button>
            <button onClick={() => setActiveTab('servicios')} className={`transition-colors ${activeTab === 'servicios' ? 'text-white border-b-2 border-indigo-500 pb-1' : 'hover:text-indigo-400'}`}>Servicios</button>
            <button onClick={() => setActiveTab('roadmap')} className={`transition-colors ${activeTab === 'roadmap' ? 'text-white border-b-2 border-indigo-500 pb-1' : 'hover:text-indigo-400'}`}>Hoja de Ruta</button>
            <button onClick={() => setActiveTab('precios')} className={`transition-colors ${activeTab === 'precios' ? 'text-white border-b-2 border-indigo-500 pb-1' : 'hover:text-indigo-400'}`}>Precios</button>
            <button onClick={() => setActiveTab('faq')} className={`transition-colors ${activeTab === 'faq' ? 'text-white border-b-2 border-indigo-500 pb-1' : 'hover:text-indigo-400'}`}>FAQ</button>
          </div>
          
          <button 
            onClick={() => setActiveTab('precios')}
            className="hidden md:block px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full text-sm font-black transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
          >
            Panel CEO
          </button>
        </div>
      ...
      {/* 📱 MENÚ MÓVIL INFERIOR (Estilo App) 📱 */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0c0a09]/95 backdrop-blur-xl border-t border-white/10 z-50 flex justify-around p-4 pb-6 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
         <button onClick={() => setActiveTab('inicio')} className={`flex flex-col items-center gap-1.5 transition-colors ${activeTab === 'inicio' ? 'text-indigo-400' : 'text-stone-500'}`}>
            <LayoutDashboard className="w-5 h-5" /><span className="text-[10px] font-black uppercase">Inicio</span>
         </button>
         <button onClick={() => setActiveTab('servicios')} className={`flex flex-col items-center gap-1.5 transition-colors ${activeTab === 'servicios' ? 'text-indigo-400' : 'text-stone-500'}`}>
            <Sparkles className="w-5 h-5" /><span className="text-[10px] font-black uppercase">Servicios</span>
         </button>
         <button onClick={() => setActiveTab('precios')} className={`flex flex-col items-center gap-1.5 transition-colors ${activeTab === 'precios' ? 'text-indigo-400' : 'text-stone-500'}`}>
            <Wallet className="w-5 h-5" /><span className="text-[10px] font-black uppercase">Planes</span>
         </button>
         <button onClick={() => setActiveTab('faq')} className={`flex flex-col items-center gap-1.5 transition-colors ${activeTab === 'faq' ? 'text-indigo-400' : 'text-stone-500'}`}>
            <HelpCircle className="w-5 h-5" /><span className="text-[10px] font-black uppercase">FAQ</span>
         </button>
      </div>

      {/* Agregamos pb-24 en móvil para que el menú inferior no tape el contenido */}
      <main className="relative z-10 pt-20 pb-24 md:pb-0">
        
        {/* ========================================================================= */}
        {/* PESTAÑA 1: INICIO (Hero y Video)                                          */}
        {/* ========================================================================= */}
        {activeTab === 'inicio' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <section className="max-w-7xl mx-auto px-6 pt-16 pb-12 lg:pt-24 text-center">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-black tracking-widest uppercase mb-12">
                <Sparkles className="w-3.5 h-3.5" /> Potencia Enterprise Web & Móvil
              </div>

              <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter mb-8 leading-[1.1] max-w-4xl mx-auto">
              ...
              <p className="text-lg md:text-xl text-stone-400 mb-16 max-w-2xl mx-auto font-medium leading-relaxed">
              ...
              <div className="relative max-w-4xl mx-auto mb-20 md:h-[650px] group">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <button onClick={() => setActiveTab('servicios')} className="w-full sm:w-auto px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black text-lg shadow-2xl shadow-indigo-600/40 flex items-center justify-center gap-3 transition-all active:scale-95">
              ...
              <div className="mt-12 flex items-center justify-center gap-6 text-sm font-bold text-stone-500">
        ...
        {/* ========================================================================= */}
        {/* PESTAÑA 2: SERVICIOS (Arquitectura, Casos de uso, Recibos)                */}
        {/* ========================================================================= */}
        {activeTab === 'servicios' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pt-8">
            <section className="py-16 bg-black/40 border-b border-white/5 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-2 bg-stone-900/50 border border-white/10 rounded-[2.5rem] p-8 relative overflow-hidden group hover:border-indigo-500/50 transition-colors duration-500 flex flex-col justify-end min-h-[300px]">
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-stone-400 leading-relaxed font-medium max-w-xl">
                        Control total desde cualquier dispositivo. Métricas en vivo, gestión matemática de membresías y capacidad de bloquear o desbloquear clientes con un solo clic.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-stone-900/50 border border-white/10 rounded-[2.5rem] p-8 group hover:border-teal-500/50 transition-colors duration-500 flex flex-col justify-center">
                    ...
                    <p className="text-sm text-stone-400 leading-relaxed">
                      Chat en tiempo real integrado. Historial de mensajes por cliente, indicador de sesión y notificaciones al instante.
                    </p>
                  </div>
                  ...
                  <div className="bg-stone-900/50 border border-white/10 rounded-[2.5rem] p-8 group hover:border-violet-500/50 transition-colors duration-500 flex flex-col justify-center">
                    ...
                    <p className="text-sm text-stone-400 leading-relaxed">
                      Frontend React protegido. Si la red falla, la app simplemente espera los datos sin colapsar.
                    </p>
                  </div>
                  ...
                  <div className="md:col-span-2 bg-gradient-to-br from-stone-900/50 to-indigo-900/10 border border-white/10 rounded-[2.5rem] p-8 relative overflow-hidden group hover:border-indigo-400/50 transition-colors duration-500 flex flex-col justify-end">
                    ...
                    <div className="relative z-10">
                      <div className="flex items-center gap-3 mb-4">
                         ...
                         <h3 className="text-2xl font-black text-white">Autenticación Inmune</h3>
                      </div>
                      <p className="text-stone-400 leading-relaxed font-medium max-w-xl">
                        Sistemas de correos sin bloqueos. Migramos a la API profesional de Brevo para enviar PINs de recuperación en milisegundos, combinado con seguridad JWT.
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {/* ========================================================================= */}
        {/* PESTAÑA 5: FAQ & CONTACTO                                                 */}
        {/* ========================================================================= */}
        {activeTab === 'faq' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pt-8">
            <section className="py-16 bg-[#0c0a09]">
      ...
      {/* FOOTER (Se muestra siempre, en todas las pestañas) */}
      <footer className="bg-black/80 py-12 relative z-10 hidden md:block">
